...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*\nPor favor não mande trava seu fdp, caso contrário, c tá fudido na mão do toddy pivete 😜✌️`
				...
				teks = `Tchau @${num.split('@')[0]} mlk entrou em shock slc, parece meu amigo Nilo😳👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ calma ae coroi, a net do toddy tá um lixo KKK⌛',
				...
				only: {
					group: '❌ Este comando só pode ser usado em grupos seu demente! ❌',
					ownerG: '⚠️ Este comando só pode ser usado pelo toddy😠✌️',
					ownerB: '❌ Este comando só pode ser usado pelo proprietário do bot! ❌',
					admin: '⚠️ A não membro comum, tentando usar comando de adms denovo? você não tem jeito mesmo KKKKK',
					Badmin: '❌ Este comando só pode ser usado quando eu virar adm porra! ❌'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["14013753811@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'Esta é a lista de filhos da putas bloqueados :\n'
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Toddy\n*YOUTUBE:* https://youtube.com/channel/UC09oDuC5ie6t2q_sLklqOfQ\n*WPP:* wa.me/+13753811\n*INSTA:* toddynho_off\n\n\n*Digite *.bot* para ver comandos basicos para criar um bot'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'belle3':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'KKKKKK '})
					break
				case 'akeno':
					meme = await kagApi.memes()
				...
				case 'loli1':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'PRAZER, SOU DA FBI, INVADI ESSE BOT DEVIDO A MENSAGEM LOLI, SINTO LHE DIZER QUE VOCÊS ESTÁ FUDIDO EM NOME DA LEI KKK'})
					break
				case 'hentai':
					memein = await kagApi.memeindo()
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do Toddy:*\n\n https://youtube.com/channel/UC09oDuC5ie6t2q_sLklqOfQ'})
					break
				case 'mia1':
					memein = await kagApi.memeindo()
				...
				case 'yt2mp3':
					...
					break
				case 'ytsearch':
					if (args.length < 1) return reply('O que você está procurando? rola?')
					...
					break
				case 'tiktok':
					if (args.length < 1) return reply('Onde está o url? Porra')
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Onde está o nome de usuário seu fdp?', text, {quoted: mek})
        ...
        case 'promote':
					...
					if (mentioned.length > 1) {
						teks = 'Mais 1 membro comum dando orgulho pro Toddy 😢\n'
						...
						mentions(`Esse ORGULHO EM PESSOA aqui @${mentioned[0].split('@')[0]} agora é admin, agr respeitem ele seus fdp`, mentioned, true)
				...
				case 'demote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Parabéns mano.... você decepcionou o toddy😓\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						...
						mentions(`Essa DECEPÇÃO em pessoa aqui @${mentioned[0].split('@')[0]} perdeu o adm, trava tá liberado nele tropa KKKK, ZUERA MEN TRAVA O CARA NÃO, mentioned, true)
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`KKKKKKKKK tomo no cu KKKKKKK : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
